...
def _get_devices(): return ["/gpu:0" if test_util.is_gpu_available() else "/cpu:0"]


def _check_device(op, expected_device="gpu"): return expected_device.upper() in op.device
...
def model_fn(sparse_vars, embed_dim, feature_inputs):
    ...
    for sp in sparse_vars:
        for inp_tensor in feature_inputs:
            embed_w, trainable = de.embedding_lookup(sp, inp_tensor, return_trainable=True)
...
def _random_weights(
    key_dtype=dtypes.int64, value_dtype=dtypes.float32, vocab_size=4, embed_dim=4, num_shards=1,
):
    assert vocab_size > 0
    assert embed_dim > 0
    assert num_shards > 0
...
class RocksDBVariableTest(test.TestCase):
    ...
    def test_basic(self):
        with self.session(config=default_config, use_gpu=False):
            table = de.get_variable(
                ...
                initializer=0,
                dim=8,
                database_path=DATABASE_PATH, embedding_name='t0_test_basic',
            )
    ...
    def test_variable(self):
        ...
        def _convert(v, t): return np.array(v).astype(_type_converter(t))
        ...
        for _id, ((key_dtype, value_dtype), dim) in enumerate(itertools.product(kv_list, dim_list)):
            ...
            with self.session(config=default_config, use_gpu=self.gpu_available):
                ...
                table = de.get_variable(
                    ...
                    dim=dim,
                    database_path=DATABASE_PATH, embedding_name='t1_test_variable',
                )
                self.evaluate(table.clear())
                ...
                remove_keys = constant_op.constant(_convert([1, 5], key_dtype), key_dtype)
                self.evaluate(table.remove(remove_keys))
                self.assertAllEqual(3, self.evaluate(table.size()))

                remove_keys = constant_op.constant(_convert([0, 1, 5], key_dtype), key_dtype)
    ...
    def test_variable_initializer(self):
        for _id, (initializer, target_mean, target_stddev) in enumerate([
            ...
            with self.session(config=default_config, use_gpu=test_util.is_gpu_available()):
                ...
                table = de.get_variable(
                    ...
                    initializer=initializer,
                    dim=10,
                    database_path=DATABASE_PATH, embedding_name='t2_test_variable_initializer',
                )
                self.evaluate(table.clear())
    ...
    def test_save_restore(self):
        ...
        with self.session(config=default_config, graph=ops.Graph()) as sess:
            ...
            table = de.Variable(
                ...
                name='t1',
                dim=1,
                database_path=DATABASE_PATH, embedding_name='t3_test_save_restore',
            )
            self.evaluate(table.clear())
        ...
        with self.session(config=default_config, graph=ops.Graph()) as sess:
            ...
            output = table.lookup(remove_keys)
            self.assertAllEqual([[-1.0], [0.0], [1.0], [2.0], [-1.0]], self.evaluate(output))
    ...
    def test_save_restore_only_table(self):
        ...
        with self.session(
            config=default_config, graph=ops.Graph(), use_gpu=test_util.is_gpu_available(),
        ) as sess:
            v0 = variables.Variable(10.0, name="v0")
            v1 = variables.Variable(20.0, name="v1")
            ...
            table = de.Variable(
                ...
                checkpoint=True,
                database_path=DATABASE_PATH, embedding_name='t4_save_restore_only_table',
            )
            self.evaluate(table.clear())
        ...
        with self.session(
            ...
            table = de.Variable(
                ...
                initializer=default_val,
                checkpoint=True,
                database_path=DATABASE_PATH, embedding_name='t6_save_restore_only_table',
            )
            self.evaluate(table.clear())

            self.evaluate(table.upsert(
    ...
    def test_training_save_restore(self):
        ...
        for _id, (key_dtype, value_dtype, dim, step) in enumerate(itertools.product(
            [dtypes.int64], [dtypes.float32], dim_list, [10],
        )):
            save_dir = os.path.join(self.get_temp_dir(), "save_restore")
            save_path = os.path.join(tempfile.mkdtemp(prefix=save_dir), "hash")

            ids = script_ops.py_func(
                _create_dynamic_shape_tensor(), inp=[], Tout=key_dtype, stateful=True,
            )
            ...
            params = de.get_variable(
                ...
                dim=dim,
                database_path=DATABASE_PATH, embedding_name='t5_training_save_restore',
            )
            ...
            with self.session(config=default_config, use_gpu=test_util.is_gpu_available()) as sess:
                ...
                for pairs_before, pairs_after in zip(
                    np_slots_kv_pairs_before_saved, np_slots_kv_pairs_after_restored
                ):
                    self.assertAllEqual(
    ...
    def test_training_save_restore_by_files(self):
        opt = de.DynamicEmbeddingOptimizer(adam.AdamOptimizer(0.3))
        for _id, (key_dtype, value_dtype, dim, step) in enumerate(itertools.product(
            [dtypes.int64], [dtypes.float32], [10], [10],
        )):
            save_dir = os.path.join(self.get_temp_dir(), "save_restore")
            save_path = os.path.join(tempfile.mkdtemp(prefix=save_dir), "hash")
            ...
            params = de.get_variable(
                ...
                initializer=0,
                dim=dim,
                database_path=DATABASE_PATH, embedding_name='t6_training_save_restore_by_files',
        ...
        with self.session(
            ...
            with variable_scope.variable_scope("embedding", reuse=True):
                table1 = de.get_variable(
                    ...
                    initializer=default_val,
                    dim=2,
                    database_path=DATABASE_PATH, embedding_name='t7_get_variable'
                )
        ...
        with self.session(
            ...
            with variable_scope.variable_scope('embedding', reuse=False):
                _ = de.get_variable(
                    ...
                    initializer=-1,
                    dim=2,
                    database_path=DATABASE_PATH, embedding_name='t8_get_variable_reuse_error',
                )
                with self.assertRaisesRegexp(ValueError, 'Variable embedding/t900 already exists'):
                    _ = de.get_variable(
                        't900',
                        initializer=-1,
                        dim=2,
    ...
    def test_sharing_between_multi_sessions(self):
        ...
        table = de.get_variable(
            ...
            initializer=0,
            dim=1,
            database_path=DATABASE_PATH, embedding_name='t9_sharing_between_multi_sessions',
        )
        self.evaluate(table.clear())
    ...
    def test_dynamic_embedding_variable(self):
        with self.session(config=default_config, use_gpu=test_util.is_gpu_available()):
            ...
            table = de.get_variable(
                ...
                initializer=default_val,
                dim=2,
                database_path=DATABASE_PATH, embedding_name='t10_dynamic_embedding_variable',
            )
            self.evaluate(table.clear())
            ...
            sorted_expected_values = np.sort([
                [4, 5],
                [2, 3],
                [0, 1]
            ], axis=0)
    ...
    def test_dynamic_embedding_variable_duplicate_insert(self):
        with self.session(use_gpu=test_util.is_gpu_available(), config=default_config) as sess:
            default_val = -1
            ...
            result = self.evaluate(output)
            self.assertTrue(list(result) in [
                [[0.0], [1.0], [3.0]],
                [[0.0], [1.0], [2.0]]
            ])
    ...
    def test_dynamic_embedding_variable_find_high_rank(self):
        with self.session(use_gpu=test_util.is_gpu_available(), config=default_config):
            default_val = -1
            keys = constant_op.constant([0, 1, 2], dtypes.int64)
            values = constant_op.constant([[0], [1], [2]], dtypes.int32)
    ...
    def test_dynamic_embedding_variable_insert_high_rank(self):
        with self.session(use_gpu=test_util.is_gpu_available(), config=default_config):
            ...
            keys = constant_op.constant([0, 1, 2], dtypes.int64)
            values = constant_op.constant([[0, 1, 2], [2, 3, 4], [4, 5, 6]], dtypes.int32)
    ...
    def test_dynamic_embedding_variables(self):
        with self.session(use_gpu=test_util.is_gpu_available(), config=default_config):
            ...
            table1 = de.get_variable(
                ...
                dtypes.int32,
                initializer=default_val,
                database_path=DATABASE_PATH, embedding_name='t191_dynamic_embedding_variables',
            )
            table2 = de.get_variable(
                ...
                dtypes.int32,
                initializer=default_val,
                database_path=DATABASE_PATH, embedding_name='t192_dynamic_embedding_variables',
            )
            table3 = de.get_variable(
                ...
                dtypes.int32,
                initializer=default_val,
                database_path=DATABASE_PATH, embedding_name='t193_dynamic_embedding_variables',
            )
            self.evaluate(table1.clear())
            self.evaluate(table2.clear())
            self.evaluate(table3.clear())
    ...
    def test_signature_mismatch(self):
        ...
        with self.session(config=config, use_gpu=test_util.is_gpu_available()):
            ...
            table = de.get_variable(
                ...
                dtypes.int32,
                initializer=default_val,
                database_path=DATABASE_PATH, embedding_name='t210_signature_mismatch',
            )
            self.evaluate(table.clear())
            ...
            with self.assertRaises(ValueError):
                self.evaluate(table.upsert(
                    constant_op.constant([4.0, 5.0, 6.0], dtypes.float32), values
                ))
