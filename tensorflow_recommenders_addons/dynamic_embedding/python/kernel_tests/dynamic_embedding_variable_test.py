...
class VariableTest(test.TestCase):
    ...
    def test_save_restore(self):
        ...
        with self.session(config=default_config, graph=ops.Graph()) as sess:
            ...
            output = table.lookup(remove_keys)
            self.assertAllEqual(
                [[-1.0], [0.0], [1.0], [2.0], [-1.0]], self.evaluate(output)
            )
        ...
        with self.session(
            ...
            with variable_scope.variable_scope("embedding", reuse=True):
                table1 = de.get_variable(
                    "t1", dtypes.int64, dtypes.int32, initializer=default_val, dim=2
                )
                table2 = de.get_variable(
                    "t1", dtypes.int64, dtypes.int32, initializer=default_val, dim=2
                )
                table3 = de.get_variable(
                    "t2", dtypes.int64, dtypes.int32, initializer=default_val, dim=2
                )
    ...
    def test_sharing_between_multi_sessions(self):
        ...
        table = de.get_variable(
            "tx100", dtypes.int64, dtypes.int32, initializer=0, dim=1
        )
    ...
    def test_dynamic_embedding_variable(self):
        with self.session(config=default_config, use_gpu=test_util.is_gpu_available()):
            ...
            values = constant_op.constant(
                [[0, 1], [2, 3], [4, 5], [6, 7]], dtypes.int32
            )
            table = de.get_variable(
                "t10", dtypes.int64, dtypes.int32, initializer=default_val, dim=2
            )
            self.assertAllEqual(0, self.evaluate(table.size()))
    ...
    def test_dynamic_embedding_variable_export_insert(self):
        with self.session(config=default_config, use_gpu=test_util.is_gpu_available()):
            ...
            table1 = de.get_variable(
                "t101", dtypes.int64, dtypes.int32, initializer=default_val, dim=2
            )
            ...
            table2 = de.get_variable(
                "t102", dtypes.int64, dtypes.int32, initializer=default_val, dim=2
            )
    ...
    def test_dynamic_embedding_variable_invalid_shape(self):
        with self.session(config=default_config, use_gpu=test_util.is_gpu_available()):
            ...
            keys = constant_op.constant([0, 1, 2], dtypes.int64)
            table = de.get_variable(
                "t110", dtypes.int64, dtypes.int32, initializer=default_val, dim=2
            )
    ...
    def test_dynamic_embedding_variable_duplicate_insert(self):
        with self.session(use_gpu=test_util.is_gpu_available(), config=default_config):
            ...
            table = de.get_variable(
                "t130", dtypes.int64, dtypes.float32, initializer=default_val
            )
            self.assertAllEqual(0, self.evaluate(table.size()))
    ...
    def test_dynamic_embedding_variable_find_high_rank(self):
        with self.session(use_gpu=test_util.is_gpu_available(), config=default_config):
            ...
            values = constant_op.constant([[0], [1], [2]], dtypes.int32)
            table = de.get_variable(
                "t140", dtypes.int64, dtypes.int32, initializer=default_val
            )
    ...
    def test_dynamic_embedding_variable_insert_low_rank(self):
        with self.session(use_gpu=test_util.is_gpu_available(), config=default_config):
            ...
            values = constant_op.constant([[[0], [1]], [[2], [3]]], dtypes.int32)
            table = de.get_variable(
                "t150", dtypes.int64, dtypes.int32, initializer=default_val
            )
    ...
    def test_dynamic_embedding_variable_remove_low_rank(self):
        with self.session(use_gpu=test_util.is_gpu_available(), config=default_config):
            ...
            values = constant_op.constant([[[0], [1]], [[2], [3]]], dtypes.int32)
            table = de.get_variable(
                "t160", dtypes.int64, dtypes.int32, initializer=default_val
            )
    ...
    def test_dynamic_embedding_variable_insert_high_rank(self):
        with self.session(use_gpu=test_util.is_gpu_available(), config=default_config):
            ...
            keys = constant_op.constant([0, 1, 2], dtypes.int64)
            values = constant_op.constant(
                [[0, 1, 2], [2, 3, 4], [4, 5, 6]], dtypes.int32
            )
            table = de.get_variable(
                "t170", dtypes.int64, dtypes.int32, initializer=default_val, dim=3
            )
    ...
    def test_dynamic_embedding_variable_remove_high_rank(self):
        with self.session(use_gpu=test_util.is_gpu_available(), config=default_config):
            ...
            table = de.get_variable(
                "t180", dtypes.int64, dtypes.int32, initializer=default_val, dim=3
            )
    ...
    def test_dynamic_embedding_variables(self):
        with self.session(use_gpu=test_util.is_gpu_available(), config=default_config):
            ...
            table1 = de.get_variable(
                "t191", dtypes.int64, dtypes.int32, initializer=default_val
            )
            table2 = de.get_variable(
                "t192", dtypes.int64, dtypes.int32, initializer=default_val
            )
            table3 = de.get_variable(
                "t193", dtypes.int64, dtypes.int32, initializer=default_val
            )
            self.evaluate(table1.upsert(keys, values))
    ...
    def test_dynamic_embedding_variable_with_tensor_default(self):
        with self.session(use_gpu=test_util.is_gpu_available(), config=default_config):
            ...
            values = constant_op.constant([[0], [1], [2]], dtypes.int32)
            table = de.get_variable(
                "t200", dtypes.int64, dtypes.int32, initializer=default_val
            )
    ...
    def test_signature_mismatch(self):
        ...
        with self.session(config=config, use_gpu=test_util.is_gpu_available()):
            ...
            values = constant_op.constant([[0], [1], [2]], dtypes.int32)
            table = de.get_variable(
                "t210", dtypes.int64, dtypes.int32, initializer=default_val
            )
    ...
    def test_dynamic_embedding_variable_int_float(self):
        with self.session(config=default_config, use_gpu=test_util.is_gpu_available()):
            ...
            table = de.get_variable(
                "t220", dtypes.int64, dtypes.float32, initializer=default_val
            )
            self.assertAllEqual(0, self.evaluate(table.size()))
    ...
    def test_dynamic_embedding_variable_with_random_init(self):
        with self.session(use_gpu=test_util.is_gpu_available(), config=default_config):
            ...
            table = de.get_variable(
                "t230", dtypes.int64, dtypes.float32, initializer=default_val
            )
