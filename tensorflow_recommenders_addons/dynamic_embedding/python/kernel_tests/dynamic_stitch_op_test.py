...
# ==============================================================================
"""Tests for ops.data_flow_ops.dynamic_stitch{_fast}."""
...
class DynamicStitchTestBase(object):
  ...
  def __init__(self, stitch_op, use_fast):
  ...
  def testScalar(self):
    with test_util.use_gpu():
      ...
      for step in -1, 1:
        stitched_t = self.stitch_op(indices[::step], data, self.use_fast)
  ...
  def testSimpleOneDimensional(self):
    ...
    for dtype in dtypes_to_test:
      ...
      data = [
          ...
          math_ops.cast(constant_op.constant([10, 60, 20, 30, 50]), dtype=dtype)
      ]
      stitched_t = self.stitch_op(indices, data, self.use_fast)
      stitched_val = self.evaluate(stitched_t)
  ...
  def testErrorIndicesMultiDimensional(self):
    ...
    with self.assertRaises(ValueError):
      self.stitch_op(indices, data, self.use_fast)
...
class DynamicStitchTest(DynamicStitchTestBase, test.TestCase):
  ...
  def __init__(self, *test_case_args):
    ...
    DynamicStitchTestBase.__init__(self, de_data_flow.dynamic_stitch, False)


class DynamicStitchTestFast(DynamicStitchTestBase, test.TestCase):

  def __init__(self, *test_case_args):
    test.TestCase.__init__(self, *test_case_args)
    DynamicStitchTestBase.__init__(self, de_data_flow.dynamic_stitch, True)
