  ...
  def common_minimize_trainable(self, base_opt, test_opt, name):
    ...
    for num_shards, k_dtype, d_dtype, initial_mode, dim, run_step in \
      ...
      with self.session(use_gpu=test_util.is_gpu_available(),
        ...
        # test graph
        embeddings = de.get_variable('t2020-' + name + str(id),
                                      key_dtype=k_dtype,
                                      value_dtype=d_dtype,
...
class EmbeddingLookupTrainableV2Test(test.TestCase, CommonTrainableTestV2Base):
  ...
  def common_minimize_trainable_v2(self, base_opt, test_opt, name):
    ...
    for num_shards, k_dtype, d_dtype, initial_mode, dim, run_step in \
      ...
      def test_fn():
        embeddings = de.get_variable('t2020-v2-' + name + str(id),
                                      key_dtype=k_dtype,
                                      value_dtype=d_dtype,
                                      devices=_get_devices() * num_shards,
                                      initializer=1.,
  ...
  def common_minimize_trainable(self, base_opt, test_opt, name):
    ...
    for num_shards, k_dtype, d_dtype, initial_mode, dim, run_step in \
      ...
      with self.session(config=default_config,
        ...
        # test var prepare
        embeddings = de.get_variable('t1030-' + name + str(id),
                                      key_dtype=k_dtype,
                                      value_dtype=d_dtype,
                                      devices=_get_devices() * num_shards,
                                      initializer=1.,
        ...
        test_var, trainable = de.embedding_lookup_sparse(embeddings,
                                                          sp_ids,
                                                          sp_weights=None,
                                                          combiner="sum",
                                                          return_trainable=True)
      ...
      # Validate updated params
      self.assertAllCloseAccordingToType(base_var_val,
                                         table_var_val,
                                         msg="Cond:{},{},{},{},{}".format(
                                             num_shards, k_dtype, d_dtype, dim,
                                             run_step))
...
class EmbeddingLookupSparseTrainableV2Test(test.TestCase,
  ...
  def common_minimize_trainable_v2(self, base_opt, test_opt, name):
    ...
    for num_shards, k_dtype, d_dtype, initial_mode, dim, run_step in \
      ...
      def test_fn():
        embeddings = de.get_variable('t1030-v2-' + name + str(id),
                                      key_dtype=k_dtype,
                                      value_dtype=d_dtype,
                                      devices=_get_devices() * num_shards,
                                      initializer=1.,
  ...
  def common_minimize_trainable(self, base_opt, test_opt, name):
    ...
    for num_shards, k_dtype, d_dtype, initial_mode, dim, run_step in \
      ...
      with self.session(config=config, use_gpu=test_util.is_gpu_available()):
        ...
        # test var prepare
        embeddings = de.get_variable('s6030-' + name + str(id),
                                      key_dtype=k_dtype,
                                      value_dtype=d_dtype,
                                      devices=_get_devices() * num_shards,
                                      initializer=1.,
...
class SafeEmbeddingLookupSparseTrainableV2Test(test.TestCase,
  ...
  def common_minimize_trainable_v2(self, base_opt, test_opt, name):
    ...
    for num_shards, k_dtype, d_dtype, initial_mode, dim, run_step in \
      ...
      def test_fn():
        embeddings = de.get_variable('s6030-v2-' + name + str(id),
                                      key_dtype=k_dtype,
                                      value_dtype=d_dtype,
                                      devices=_get_devices() * num_shards,
                                      initializer=1.,
