...
class CommonTrainableTestV1Base(object):
    ...
    def test_rmsprop_minimize_trainable(self):
        for centered_ in [False, True]:
            ...
            self.common_minimize_trainable(
                base_opt, test_opt, name="rmsprop" + str(centered_)
            )
        ...
        for (
            ...
            with self.session(
                ...
                x = constant_op.constant(
                    np.random.rand(dim, len(raw_ids)), dtype=d_dtype
                )
                ...
                self.assertAllCloseAccordingToType(
                    raw_init_vals[raw_ids], self.evaluate(test_var)
                )
                ...
                self.assertAllCloseAccordingToType(
                    ...
                    msg="Cond:{},{},{},{},{},{}".format(
                        num_shards, k_dtype, d_dtype, initial_mode, dim, run_step
                    ),
                )
    ...
    def common_minimize_trainable_v2(self, base_opt, test_opt, name):
        ...
        for (
            ...
            def base_fn():
                ...
                base_opt_op = base_opt.minimize(
                    lambda: loss_fn(embeddings), [embeddings]
                )
                self.evaluate(variables.global_variables_initializer())
                for _ in range(run_step):
...
class EmbeddingLookupSparseTrainableV1Test(test.TestCase, CommonTrainableTestV1Base):
    def common_minimize_trainable(self, base_opt, test_opt, name):
        base_opt = de.DynamicEmbeddingOptimizer(base_opt)
        test_opt = de.DynamicEmbeddingOptimizer(test_opt)
        id = 0
        ...
        for (
            ...
            x = constant_op.constant(
                [[_x * dim] for _x in [[0.4], [0.5], [0.6]]], dtype=d_dtype
            )
...
class EmbeddingLookupSparseTrainableV2Test(test.TestCase, CommonTrainableTestV2Base):
    def common_minimize_trainable_v2(self, base_opt, test_opt, name):
        base_opt = de.DynamicEmbeddingOptimizer(base_opt)
        test_opt = de.DynamicEmbeddingOptimizer(test_opt)
        id = 0
        for (
            ...
            def base_fn():
                ...
                def loss_fn(emb):
                    embedding = embedding_ops.embedding_lookup_sparse(
...
class SafeEmbeddingLookupSparseTrainableV1Test(
    test.TestCase, CommonTrainableTestV1Base
):
    @test_util.deprecated_graph_mode_only
    def common_minimize_trainable(self, base_opt, test_opt, name):
        base_opt = de.DynamicEmbeddingOptimizer(base_opt)
        ...
        for (
            ...
            with self.session(config=config, use_gpu=test_util.is_gpu_available()):
                ...
                table_var = array_ops.reshape(
                    embeddings.lookup(init_ids), shape=[10, dim]
                )
                # Validate updated params
                self.assertAllCloseAccordingToType(
                    self.evaluate(base_var),
...
class SafeEmbeddingLookupSparseTrainableV2Test(
    test.TestCase, CommonTrainableTestV2Base
):
    def common_minimize_trainable_v2(self, base_opt, test_opt, name):
        base_opt = de.DynamicEmbeddingOptimizer(base_opt)
        test_opt = de.DynamicEmbeddingOptimizer(test_opt)
    ...
    def test_saving_restoring_checkpoint(self):
        ...
        with ops.Graph().as_default():
            ...
            with monitored_session.MonitoredTrainingSession(
                config=default_config, is_chief=True, checkpoint_dir=logdir
            ) as sess:
            ...
            with monitored_session.MonitoredTrainingSession(
                ...
                self.assertAllEqual(
                    target_values, sess.run(lookup_op, feed_dict={keys: [0, 1, 2]})
                )
    ...
    def common_minimize_trainable(self, base_opt, test_opt, name):
        ...
        for (
            ...
            with ops.Graph().as_default():
                ...
                gstep = training_util.create_global_step()
                test_opt_op = test_opt.minimize(
                    loss1, var_list=[trainable], global_step=gstep
                )
                ...
                with monitored_session.MonitoredTrainingSession(
                    is_chief=True, config=default_config
                ) as sess:
                    sess.run(init_op)
                    self.assertAllCloseAccordingToType(
                        np.array(raw_init_vals).reshape([len(raw_init_ids), dim]),
