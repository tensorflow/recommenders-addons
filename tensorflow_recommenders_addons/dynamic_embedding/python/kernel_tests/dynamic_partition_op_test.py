...
import tensorflow_recommenders_addons.dynamic_embedding.python.ops.data_flow_grad  # pylint: disable=unused-import
...
class DynamicPartitionTest(test.TestCase):
  ...
  def testSimpleOneDimensional(self):
    with self.session():
      ...
      partitions = data_flow.dynamic_partition(data, indices, num_partitions=4)
  ...
  def testLargeOneDimensional(self):
    ...
    with self.session():
      ...
      partitions = data_flow.dynamic_partition(data, indices, num_partitions=2)
      partition_vals = self.evaluate(partitions)
  ...
  def testLargeTwoDimensional(self):
    ...
    with self.session():
      data = constant_op.constant(data_list, dtype=dtypes.float32)
      indices = constant_op.constant(indices_list, dtype=dtypes.int32)
      partitions = data_flow.dynamic_partition(data,
                                               indices,
      ...
      for n in 2, 3:
        for shape in (4,), (4, 5), (4, 5, 2):
          ...
          for extra_shape in (), (6,), (6, 7):
            ...
            outputs = data_flow.dynamic_partition(data_t,
                                                  partitions_t,
                                                  num_partitions=n)
  ...
  def testEmptyDataTwoDimensional(self):
    ...
    with self.session():
      data = constant_op.constant(data_list, dtype=dtypes.float32)
      indices = constant_op.constant(indices_list, dtype=dtypes.int32)
      partitions = data_flow.dynamic_partition(data, indices, num_partitions=3)
      partition_vals = self.evaluate(partitions)
  ...
  def testGPUPartsTooLarge(self):
    ...
    with self.session():
      data = constant_op.constant(data_list, dtype=dtypes.float32)
      indices = constant_op.constant(indices_list, dtype=dtypes.int32)
      partitions = data_flow.dynamic_partition(data, indices, num_partitions=5)
      partition_vals = self.evaluate(partitions)
  ...
  def testGPUAllIndicesBig(self):
    ...
    with self.session():
      data = constant_op.constant(data_list, dtype=dtypes.float32)
      indices = constant_op.constant(indices_list, dtype=dtypes.int32)
      partitions = data_flow.dynamic_partition(data, indices, num_partitions=40)
      partition_vals = self.evaluate(partitions)
  ...
  def testScalarIndexOutOfRange(self):
    ...
    with self.cached_session(use_gpu=False):
      bad = 17
      data = np.zeros(5)
      partitions = data_flow.dynamic_partition(data, bad, num_partitions=7)
  ...
  def testHigherRankIndexOutOfRange(self):
    ...
    with self.cached_session(use_gpu=False) as sess:
      ...
      partitions = data_flow.dynamic_partition(data, indices, num_partitions=7)
      for i in range(2):
        for j in range(3):
  ...
  def testErrorWrongDimsIndices(self):
    ...
    with self.assertRaises(ValueError):
      data_flow.dynamic_partition(data, indices, num_partitions=4)
  ...
  def testCUBBug(self):
    ...
    partitioned = data_flow.dynamic_partition(x, inds, 16)
  ...
  def testMultiGPU(self):
    ...
    for device in device_list:
      with ops.device(device.name):
        ...
        result = data_flow.dynamic_partition(data, partitions, 10)
