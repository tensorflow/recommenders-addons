...
class CuckooHashTable(LookupInterface):
  ...
  def __init__(self,
    ...
    if checkpoint:
      saveable = CuckooHashTable._Saveable(self, name)
  ...
  def export(self, name=None):
    ...
    with ops.name_scope(name, "%s_lookup_table_export_values" % self.name,
      ...
      with ops.colocate_with(self.resource_handle):
        exported_keys, exported_values = cuckoo_hashtable_ops.cuckoo_hash_table_export(
  ...
  def _gather_saveables_for_checkpoint(self):
    ...
    return {
        "table":
            functools.partial(CuckooHashTable._Saveable,
                              table=self,
                              name=self._name,
                              full_name=full_name)
    }
