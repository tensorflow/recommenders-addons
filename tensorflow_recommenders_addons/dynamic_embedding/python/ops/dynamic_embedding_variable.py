...
def _partition(data, partition_index, shard_num):
    ...
    with ops.colocate_with(data, ignore_existing=True):
        partitions = data_flow_ops.dynamic_partition(data, partition_index, shard_num)
...
def default_partition_fn(keys, shard_num):
    ...
    with ops.colocate_with(keys_op):
        if keys_op.dtype == dtypes.int64:
            ...
            keys_int32 = math_ops.cast(
                bitwise_ops.bitwise_and(keys_op, mask), dtypes.int32
            )
            mod = math_ops.mod(
                keys_int32, constant_op.constant(shard_num, dtypes.int32)
            )
...
class Variable(trackable.TrackableResource):
    ...
    def __init__(
        ...
        if "GPU" in self.devices[0].upper():
            ...
            value_dtype_list = [dtypes.int32, dtypes.float32, dtypes.half, dtypes.int8]
    ...
    def _convert_anything_to_init(self, raw_init, dim):
        ...
        while callable(init):
            if isinstance(init, (init_ops.Initializer, init_ops_v2.Initializer)):
                ...
                init = init()
        init = math_ops.cast(
            array_ops.fill([dim], array_ops.reshape(init, [-1])[0]),
    ...
    def _make_name(self, table_idx):
        return "{}_mht_{}of{}".format(
            self.name.replace("/", "_"), table_idx + 1, self.shard_num
        )
    ...
    def upsert(self, keys, values, name=None):
        ...
        for idx in range(len(self.devices)):
            with ops.device(self.devices[idx]):
                ops_.append(
                    self._tables[idx].insert(
                        keys_partitions[idx], values_partitions[idx], name=name
                    )
                )
    ...
    def _create_default_values_by_initializer(self, keys):
        ...
        try:
            ...
            tf_logging.warn(
                "Variable [{}] is not running on full-size initialization mode: {}".format(
                    str(self.name), str(e)
                )
            )
        return init_op
    ...
    def lookup(self, keys, name=None):
        ...
        """
        partition_index = self.partition_fn(keys, self.shard_num)
        keys_partitions, keys_indices = _partition(
            keys, partition_index, self.shard_num
        )
    ...
    def size(self, index=None, name=None):
        ...
        return (
            self.size_ops[index] if index is not None else math_ops.add_n(self.size_ops)
        )
