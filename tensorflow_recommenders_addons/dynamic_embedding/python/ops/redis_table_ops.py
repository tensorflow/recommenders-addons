...
class RedisTable(LookupInterface):
    ...
    def __init__(
        ...
        os.environ.redis_wait_timeout = str(
            self._redis_params.redis_wait_timeout)
        ...
        with ops.name_scope(
            ...
            with ops.colocate_with(self.resource_handle):
                values = redis_table_ops.tfra_redis_table_find(
                    self.resource_handle,
                    keys,
                    dynamic_default_values if dynamic_default_values is not None
    ...
    def import_from_flies(self, name=None):
        ...
        with ops.name_scope(name, "%s_lookup_table_export_values" % self.name,
            ...
            with ops.colocate_with(self.resource_handle):
                ...
                if ndims == 1:
                    useless_keys = ops.convert_to_tensor(0,
                                                         dtype=self.key_dtype)
                else:
                    useless_keys = tf.zeros(
                        shape=(self._default_value.get_shape()[0]),
                        dtype=self.key_dtype)
                useless_vals = tf.zeros(shape=self._default_value.get_shape(),
                                        dtype=self.value_dtype)
                return redis_table_ops.tfra_redis_table_import(
                    self.resource_handle, useless_keys, useless_vals)
