...
def DynamicEmbeddingOptimizer(self):
                    ...
                    if isinstance(grad, ops.IndexedSlices):
                        ...
                        with ops.control_dependencies([_apply_op]):
                            _after = control_flow_ops.group(
                                [var.update_op()] + [_s.update_op() for _s in _slots]
                            )
                            return _after
        ...
        if weight is None:
            ...
            with strategy.extended.colocate_vars_with(var):
                if isinstance(var, de.TrainableWrapper):
                    weight = de.create_slots(
                        var, initial_value, slot_name, var._shared_name
                    )
                else:
    ...
    def _get_or_make_slot_with_initializer(
        var, initializer, shape, dtype, slot_name, op_name
    ):
        ...
        if optimizer._var_key(var) not in named_slots:
            if isinstance(var, de.TrainableWrapper):
                new_slot_variable = de.create_slots(
    ...
    if isinstance(self, optimizer.Optimizer):
        ...
        raise Exception(
            "Optimizer type is not supported! got {}".format(str(type(self)))
        )
    return self
