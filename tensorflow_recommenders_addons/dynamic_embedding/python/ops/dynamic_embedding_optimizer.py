...
def DynamicEmbeddingOptimizer(self, bp_v2=False, horovod_synchronous=False):
  ...
  Args:
    ...
    horovod_synchronous: If True, we will use horovod's all-reduce method to merge the dense grad of model parameter, 
  ...
  """
  self._bp_v2 = bp_v2
  self._hvd_sync = horovod_synchronous
  ...
  def apply_gradients(grads_and_vars, global_step=None, name=None):
    ...
    try:
      ...
      raise ValueError(
          "Please install Horovod first if you want to use distributed synchronous training ased on Horovod"
      )
