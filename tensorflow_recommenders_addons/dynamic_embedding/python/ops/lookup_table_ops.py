...
from tensorflow_recommenders_addons.utils.resource_loader import get_path_to_datafile

lookup_table_ops = LazySO("dynamic_embedding/core/_lookup_table_ops.so").ops
...
class LookupTable(LookupInterface):
  """
    ...
    ```python
    table = tfra.dynamic_embedding.LookupTable(key_dtype=tf.string,
  ...
  def __init__(
    ...
    """
      ...
      Creates a lookup table through OS envionment variables,
    ...
    lookup_table_library = None

    if not self._lookup_table_name:
      self._lookup_table_name = os.getenv('LOOKUP_TABLE_NAME')
    
    lookup_table_library = lookup_table_dict.get(self._lookup_table_name)
    if not lookup_table_library:
      lookup_table_library = lookup_table_dict.get('STAND_REDIS')

    tf.load_library(get_path_to_datafile(lookup_table_library))
    ...
    if checkpoint:
      _ = LookupTable._Saveable(self, name)
      ...
      if shard_saveable_object_fn:
        ...
        self._saveable_fn = LookupTable._Saveable
  ...
  def _create_resource(self):
    ...
    self._redis_config_abs_dir = ''
    self._redis_config_abs_dir_env = ''
    if self._config:
      self._redis_config_abs_dir = self._config.redis_config_abs_dir
      self._redis_config_abs_dir_env = self._config.redis_config_abs_dir_env

    with ops.device(self._device):
      table_ref = lookup_table_ops.tfra_lookup_table_of_tensors(
          ...
          redis_config_abs_dir=self._redis_config_abs_dir,
          redis_config_abs_dir_env=self._redis_config_abs_dir_env,
  ...
  def size(self, name=None):
    ...
    with ops.name_scope(name, "%s_Size" % self.name, [self.resource_handle]):
      with ops.colocate_with(self.resource_handle):
        return lookup_table_ops.tfra_lookup_table_size(self.resource_handle)
  ...
  def remove(self, keys, name=None):
    ...
    with ops.name_scope(
      ...
      op = lookup_table_ops.tfra_lookup_table_remove(self.resource_handle, keys)
  ...
  def clear(self, name=None):
    ...
    with ops.name_scope(name, "%s_lookup_table_clear" % self.name,
      ...
      op = lookup_table_ops.tfra_lookup_table_clear(self.resource_handle,
                                                  key_dtype=self._key_dtype,
                                                  value_dtype=self._value_dtype)
  ...
  def lookup(self,
    ...
    with ops.name_scope(
      ...
      with ops.colocate_with(self.resource_handle):
        if return_exists:
          values, exists = lookup_table_ops.tfra_lookup_table_find_with_exists(
          ...
          values = lookup_table_ops.tfra_lookup_table_find(
              self.resource_handle,
              keys,
              dynamic_default_values
              if dynamic_default_values is not None else self._default_value,
  ...
  def insert(self, keys, values, name=None):
    ...
    with ops.name_scope(
      ...
      with ops.colocate_with(self.resource_handle):
        # pylint: disable=protected-access
        op = lookup_table_ops.tfra_lookup_table_insert(self.resource_handle, keys,
                                                     values)
    return op
  ...
  def accum(self, keys, values_or_deltas, exists, name=None):
    ...
    with ops.name_scope(
      ...
      with ops.colocate_with(self.resource_handle):
        # pylint: disable=protected-access
        op = lookup_table_ops.tfra_lookup_table_accum(self.resource_handle, keys,
    ...
    with ops.name_scope(name, "%s_lookup_table_export_values" % self.name,
      ...
      with ops.colocate_with(self.resource_handle):
        (
            exported_keys,
            exported_values,
        ) = lookup_table_ops.tfra_lookup_table_export(self.resource_handle,
                                                    self._key_dtype,
                                                    self._value_dtype)
  ...
  def save_to_file_system(self,
    ...
    with ops.name_scope(name, "%s_save_table" % self.name,
      ...
      with ops.colocate_with(None, ignore_existing=True):
        return lookup_table_ops.tfra_lookup_table_save_to_file_system(
            self.resource_handle,
            dirpath=dirpath,
  ...
  def load_from_file_system(self,
    ...
    with ops.name_scope(name, "%s_load_table" % self.name,
                        [self.resource_handle]):
      with ops.colocate_with(None, ignore_existing=True):
        return lookup_table_ops.tfra_lookup_table_load_from_file_system(
            self.resource_handle,
            dirpath=dirpath,
            file_name=file_name if file_name else self._name,
            key_dtype=self._key_dtype,
  ...
  class _Saveable(BaseSaverBuilder.SaveableObject):
    """SaveableObject implementation for LookupTable."""
    ...
    def __init__(self, table, name, full_name=""):
      ...
      # pylint: disable=protected-access
      super(LookupTable._Saveable, self).__init__(table, specs, name)
    ...
    def restore(self, restored_tensors, restored_shapes, name=None):
      ...
      with ops.name_scope(name, "%s_table_restore" % self._restore_name):
        with ops.colocate_with(self.op.resource_handle):
          return lookup_table_ops.tfra_lookup_table_import(
...
ops.NotDifferentiable(prefix_op_name("LookupTableOfTensors"))