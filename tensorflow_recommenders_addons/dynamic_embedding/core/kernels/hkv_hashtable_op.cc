...
using namespace tensorflow::recommenders_addons::lookup;
...
template <class K, class V>
class HkvHashTableOfTensors final : public LookupInterface {
 public:
  HkvHashTableOfTensors(OpKernelContext* ctx, OpKernel* kernel) {
    int64 env_var = 0;
    int64 init_size = 0;
    OP_REQUIRES_OK(ctx,
                   GetNodeAttr(kernel->def(), "value_shape", &value_shape_));
    OP_REQUIRES_OK(ctx,
                   GetNodeAttr(kernel->def(), "init_capacity", &init_size));
    OP_REQUIRES(
  ...
  ~HkvHashTableOfTensors() { delete table_; }
  ...
  int64 MemoryUsed() const override {
    ...
    return sizeof(HkvHashTableOfTensors) + ret;
  }
...
REGISTER_KERNEL_BUILDER(
    Name(PREFIX_OP_NAME(HkvHashTableFind)).Device(DEVICE_CPU), HashTableFindOp);
REGISTER_KERNEL_BUILDER(
    Name(PREFIX_OP_NAME(HkvHashTableInsert)).Device(DEVICE_CPU),
    HashTableInsertOp);
REGISTER_KERNEL_BUILDER(
    Name(PREFIX_OP_NAME(HkvHashTableRemove)).Device(DEVICE_CPU),
    HashTableRemoveOp);
REGISTER_KERNEL_BUILDER(
    Name(PREFIX_OP_NAME(HkvHashTableSize)).Device(DEVICE_CPU), HashTableSizeOp);
REGISTER_KERNEL_BUILDER(
    Name(PREFIX_OP_NAME(HkvHashTableExport)).Device(DEVICE_CPU),
    HashTableExportOp);
REGISTER_KERNEL_BUILDER(
    Name(PREFIX_OP_NAME(HkvHashTableImport)).Device(DEVICE_CPU),
    HashTableImportOp);
...
#define REGISTER_KERNEL(key_dtype, value_dtype)                               \
  REGISTER_KERNEL_BUILDER(                                                    \
      ...
      HashTableOp<hkv_table::HkvHashTableOfTensors<key_dtype, value_dtype>,   \
