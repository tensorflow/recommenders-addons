...
namespace tensorflow {
  namespace recommenders_addons {
    namespace rocksdb_lookup {
      ...
      public:
        HashTableOp(OpKernelConstruction *ctx)
        : OpKernel(ctx), table_handle_set_(false) {
          ...
          OP_REQUIRES_OK(ctx, ctx->GetAttr(
            "use_node_name_sharing", &use_node_name_sharing_
          ));
        }
        ...
        void Compute(OpKernelContext *ctx) override {
          ...
          auto creator = [ctx, this](lookup::LookupInterface **ret) TF_EXCLUSIVE_LOCKS_REQUIRED(mu_) {
          ...
          OP_REQUIRES_OK(
            ctx,
            cinfo_.resource_manager()->template LookupOrCreate<lookup::LookupInterface>(
          ...
          OP_REQUIRES_OK(ctx, CheckTableDataTypes(
            *table, DataTypeToEnum<key_dtype>::v(),
            DataTypeToEnum<value_dtype>::v(),
            cinfo_.name()
          ));

          if (ctx->expected_output_dtype(0) == DT_RESOURCE) {
            if (!table_handle_set_) {
              auto h = table_handle_.AccessTensor(ctx)->template scalar<ResourceHandle>();
        ...
        ~HashTableOp() override {
          if (table_handle_set_ && cinfo_.resource_is_private_to_kernel()) {
            if (!cinfo_.resource_manager()->template Delete<lookup::LookupInterface>(
