...
function main() {
  ...
  echo "=== Copy TensorFlow Recommenders Addons files"
  ...
  if is_windows; then
    from=$(cygpath -w ${PIP_FILE_PREFIX}tensorflow_recommenders_addons)
    to=$(cygpath -w "${TMPDIR}"/tensorflow_recommenders_addons)
    start robocopy //S "${from}" "${to}" //xf *_test.py
    sleep 5
  else
    rsync -avm -L --exclude='*_test.py' ${PIP_FILE_PREFIX}tensorflow_recommenders_addons "${TMPDIR}"
  fi
